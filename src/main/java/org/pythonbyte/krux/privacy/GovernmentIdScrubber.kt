...
class GovernmentIdScrubber : PIIScrubber {
    override fun scrub(str: String): String {
        ...
        result =
            ssnContextRegex.replace(result) { matchResult ->
                val prefix = matchResult.groupValues[1]
                "$prefix[SSN REDACTED]"
            }

        result =
            sinContextRegex.replace(result) { matchResult ->
                val prefix = matchResult.groupValues[1]
                "$prefix[SIN REDACTED]"
            }
        ...
        result =
            ssnGenericRegex.replace(result) { matchResult ->
        ...
        result =
            sinGenericRegex.replace(result) { matchResult ->
                val number = matchResult.groupValues[1]
                if (number.length == 9 && !(number.startsWith("0") || number.startsWith("8") || number.startsWith("9"))) {
                    "[SIN REDACTED]"
                } else if (number.matches(Regex("\\d{3}[\\s-]?\\d{3}[\\s-]?\\d{3}"))) {
